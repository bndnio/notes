...
Production secrets live in Cloudflare Secrets Store. Local `wrangler dev` uses a separate local store — `.dev.vars` alone is not enough for `secrets_store_secrets` bindings.

1. Copy `.dev.vars.example` to `.dev.vars`
2. Set dummy values for `RESEND_API_KEY` / `NOTION_CLIENT_SECRET` if unused locally (`ENCRYPTION_KEY` is auto-generated by `setup-dev` if missing)
3. Run `bun run setup-dev` to sync `.dev.vars` into the local Secrets Store (before `bun run dev`)
