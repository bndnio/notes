...
| Secrets Store | Local store under `.wrangler/` |
...
**`.dev.vars`** — gitignored local config. At minimum set `APP_URL=http://localhost:8787` and dummy values for `RESEND_API_KEY` / `NOTION_CLIENT_SECRET`. `ENCRYPTION_KEY` is auto-generated by `setup-dev` if missing or still set to the placeholder.

**`bun run setup-dev`** — reads `.dev.vars` and syncs secrets into the **local** Secrets Store (`wrangler secrets-store secret create ... --remote=false`). Production secrets are not used and `.dev.vars` alone is not enough for `secrets_store_secrets` bindings — they must be loaded into the local store this way.
...
Login/register PINs are logged in the **terminal running wrangler**, not the browser console, when `APP_URL` points at localhost.

Re-run `bun run setup-dev` after changing secrets in `.dev.vars`.
